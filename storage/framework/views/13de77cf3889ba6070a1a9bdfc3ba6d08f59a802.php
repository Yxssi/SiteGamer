        ...
        <div class="container">
                            ...
                            <tbody>
                                ...
                                <tr>
                                    ...
                                    <td class="border-0 align-middle">
                                        <select class="custom-select" name="qty" id="qty" data-id="<?php echo e($product->rowId); ?>" data-stock="<?php echo e($product->model->stock); ?>">
                                            <?php for($i = 1; $i <= 5; $i++): ?>
            ...
            <div class="row p-4 bg-white rounded shadow-sm">
                <div class="col-lg-6">
                    ...
                    <form action="<?php echo e(route('cart.store.coupon')); ?>" method="POST">
                        <?php echo csrf_field(); ?>
                ...
                <div class="col-lg-6">
                    ...
                    <div class="p-4">
                        ...
                        <?php if(request()->session()->has('coupon')): ?>
                        <li class="d-flex justify-content-between py-3 border-bottom"><strong class="text-muted">Coupon <?php echo e(request()->session()->get('coupon')['code']); ?>


                        <form action="<?php echo e(route('cart.destroy.coupon')); ?>" method="POST" class="d-inline-block">
                            <?php echo csrf_field(); ?>
                            <?php echo method_field('DELETE'); ?>
                            <button type="submit" class="btn btn-sm btn-outline-danger"><i class="fa fa-trash"></i></button>
                        </form>
                        </strong><strong><?php echo e(getPrice(request()->session()->get('coupon')['remise'])); ?></strong></li>
                        <li class="d-flex justify-content-between py-3 border-bottom"><strong class="text-muted">Nouveau Sous-total</strong><strong>
                        <?php echo e(getPrice(Cart::subtotal() - request()->session()->get('coupon')['remise'])); ?>

                        </strong></li>
                        <li class="d-flex justify-content-between py-3 border-bottom"><strong class="text-muted">Taxe</strong><strong>
                        <?php echo e(getPrice((Cart::subtotal() - request()->session()->get('coupon')['remise']) * (config('cart.tax') / 100))); ?>

                        </strong></li>
                        <li class="d-flex justify-content-between py-3 border-bottom"><strong class="text-muted">Total</strong><strong>
                        <?php echo e(getPrice((Cart::subtotal() - request()->session()->get('coupon')['remise']) +(Cart::subtotal() - request()->session()->get('coupon')['remise']) * (config('cart.tax') / 100))); ?>
...
<?php echo $__env->make('layouts.master', \Illuminate\Support\Arr::except(get_defined_vars(), ['__data', '__path']))->render(); ?><?php /**PATH /Applications/MAMP/htdocs/serverphp/e-commerce-master/Gamerz/resources/views/cart/index.blade.php ENDPATH**/ ?>